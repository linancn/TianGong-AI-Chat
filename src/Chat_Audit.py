...
try:
    ...
    with st.sidebar:
        ...
        st.subheader(
            "您的智能审计助手",
            help="Intellegent Audit Assistant",
        )
        ...
        with st.expander(ui.sidebar_expander_title, expanded=True):
            ...
            search_public = st.toggle(
                ui.search_public_checkbox_label,
                value=False,
                disabled=st.session_state["search_option_disabled"],
            )
            search_internal = st.toggle(
                ui.search_internal_checkbox_label,
                value=False,
                disabled=st.session_state["search_option_disabled"],
            )
        ...
        if "xata_history_refresh" not in st.session_state:
            ...
            if user_query:
                ...
                if answer is not None:
                    ...
                    if search_public or search_internal:

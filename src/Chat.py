                        ...
                        if (
                            ...
                            if answer is not None:
                                ...
                                if (
                                    ...
                                    input = f"""回应“{user_query}”以用户相同的语言提供逻辑清晰、经过批判性分析的回复。必须：
    - 根据最近的对话上下文“{chat_history_recent}”（若有）以及搜索到的相关信息（若有）中的有效信息（避免使用不相关信息）如下：{docs_response}。
    - 确保所有数学公式前后使用“\[”和“\]”符号包裹同时必须引用LaTeX语法使用标记数学公式，以确保公式在Markdown中正确渲染。
